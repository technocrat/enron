...
For natural language processing (**NLP**) purposes, treating the `payload` rather than the `body` as the unit of analysis avoided an *echo chamber* effect of threads quoting and re-quoting the original message, multiplying the frequency of the words it contained.
...
To achieve a computationally practicable dataset for social network analysis, emails were limited to single Enron sender to Enron single recipient, reducing the dataset further, to 9,615 emails. The resulting graph had low density, and the the dataset was further trimmed by restricting it to reciprocal correspondents, each of whom sent an email to the other, either as a reply or an original message, excluding emails by a user to herself.. This further reduced the dataset, to 6,009 emails among 465 users.

## Social network analysis

### The nature of social networks

Following the reduction of the corpus, the remaining senders and receivers were humans who engaged in mutual correspondence. The humans constitute `nodes` or `verticess` and their emails `edges`^[Or `arcs` when directionality is considered]. Draw theee points and connect them, and you have created three nodes and three edges, a triagle, which is termed a `graph` object. A graph object encapsulates many useful features aside from who knows whom^[Such as the parlor game [six degrees of Kevin Bacon]] including measures of density, centrality, connectedness, separation, clustering and other measures of how well or poorly embedded in an organization any individual's email record reveals.
...
net_p0 <- plot(net, vertex.cex = 0.1)
```
...
```{r netsize, echo=FALSE, results = 'asis'}
net
net_edgeno <- length(net$mel)
net_users <- g_enron %>% distinct(f_userid) %>% nrow()
net_vertices <- net$gal$n
```

The graph consists of `r net_edgeno` edges, each representing one email, an average of `r net_edgeno/net_users` emails per user.

#### Density

```{r density_0, echo=FALSE}
net0 <- gden(net) 
```

The graph has a density of `r net0`, which is very low.
...
As seen in the visualization of the graph, two components are present, one that is strongly connected, and the other, weakly, comprising `r net_strong` and `r net_weak` members, respectively.
...
### The Reduced Enron graph

```{r extract, echo=FALSE, results='hide'}
net_r <- delete.vertices(net, isolates(net))
net_r_vertices <- net_r$gal$n
```

Eliminating isolates reduces the number of connection endpoints from `r net_vertices` to `r net_r_vertices`.

#### Visualization

```{r p1, echo=FALSE}
net_p0 <- plot(net_r, vertex.cex = 0.75)
...
The graph has a density of `r net1`,  compared to the original graph, `r net0` or `r net1/net0` greater.
...
### Social network analysis of the reduced Enron graph
