...
library(coda)
load("net_p.fit.Rda")
mcmc.diagnostics(net_p.fit, which.diags = c("cor", "acf", "raftery"))
```
...
n2_words <- n2_text %>% unnest_tokens(word, payload)  %>% anti_join(stop_words)
n3_words <- n3_text %>% unnest_tokens(word, payload) %>% anti_join(stop_words)

n1_words %>% count(word, sort = TRUE) %>% print(n = 50)
