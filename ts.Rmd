...
# find the users with the top n evcent reduced 
# # in a graph object	x
evctr <-  function(x,n) {
	enframe(evcent(x, use.eigen = TRUE))  %>%
...
# find the users with the top n loadcent reduced 
# # in a graph object	x
ldctr <-  function(x,n) {
...
# find the users with the top n degree reduced
# in a graph object	x
deg <-  function(x,n) {
...
```{r, echo=FALSE}
kable(ldctr(Qall,10)) %>%   kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
ggnet2(Qall, label = ldctr(Qall,10)$userid, label.size = 4, label.color = "darkblue", node.size = 0.25, edge.size = 0.25, edge.color = "grey")
top_ldctr <- ldctr(Qall,10)$userid
```

### Eigenvector centrality

This measures the centrality of a vertex as a function of the centrality of the vertices to which it is connected, taking into account, in turn, the centrality of *those vertices.*
...
top_evctr <- evctr(Qall,10)$userid
```
...
```{r, echo = FALSE}
central_users <- (union(union(top_deg,top_ldctr),top_evctr))
...
Following inspection, the Qall 533 vertex graph was reduced by  
170 vertices by selecting the rows at which the cumulative density of load centrality and Eigenvector centrality are both above densities of 0.5. The degree centrality is highly concentrated within the first 35 vetices.

```{r, echo=FALSE}
# calculate cumulative sums of normalized centrality measures

ctr_scores <-prominence         %>%
  rownames_to_column(var = "id")%>% 
  arrange(desc(deg, ldc, evc))            %>%
  mutate(deg_sum = cumsum(deg)) %>%
  mutate(ldc_sum = cumsum(ldc)) %>%
  mutate(evc_sum = cumsum(evc)) %>%
  filter(id > 170)              %>%
  select(userid)
kable(ctr_) %>%  kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
```
...
#### Application of the deconstruction of the core Enron network
