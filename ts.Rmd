...
# load("g_enron.Rda") checkpoint version between sessions
...
As illustrated by the quarterly tables and plot above, the number of users participating in the social network varied over time. However, only users `r All_qtrs` have activity in all seven quarters. 

Alternatively there are top-25 lists of emails from one user to another

```{r, echo=FALSE}
kable(top_n(msg_count,25)) %>%   kable_styling(bootstrap_options = "striped", full_width = F, position = "center")

```
The users `r All_qtrs` appear `r sum(All_qtrs %in% msg_count$s_uid[1:10])` among time among them as senders and `r sum(All_qtrs %in% msg_count$r_uid[1:10])` times as recipients.
...
The users `r All_qtrs` appear among them `r sum(All_qtrs %in% s_uid_count$s_uid[1:10])` times.
...
The users `r All_qtrs` appear among them `r sum(All_qtrs %in% r_uid_count$r_uid[1:10])` times as recipients.

Only one user of the three most frequent email users was among the users in the top-pairs list, the top senders list and the top receivers list. Another was in the top senders list and the top receivers list. The third was missing from all three lists. 

### Graph measures of user connectedness

#### Degree

`Degree` is one of many metrics that measure the relative position in a graph of a vertex, or user. It is simply the number of connections with different users. The top 25 users ranked by that criterion are:

```{r, echo=FALSE}
kable(deg(Qall,25)) %>%   kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
```
### Closeness
