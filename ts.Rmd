...
# # in a graph object	x
# betw <-  function(x,n) {
# 	enframe(betweenness(x, rescale = TRUE)) %>% 
...
# # in a graph object	x
# clos <-  function(x,n) {
# 	enframe(closeness(x, cmode="suminvdir", rescale = TRUE)) %>% 
# 	mutate(userid = userid$userid[name]) 	%>%
# 	select(userid, value)			        		%>%
...
# find the users with the top n loadcent scores 
# # in a graph object	x
ldctr <-  function(x,n) {
	enframe(loadcent(x, rescale = TRUE))  %>%
	mutate(userid = userid$userid[name]) 	%>%
	select(userid, value)			        		%>%
	rename(loadcent = value)   				    %>%
	arrange(desc(loadcent)) 					    %>%
	top_n(n)
}
# find the users with the top n degree scores 
# in a graph object	x
deg <-  function(x,n) {
	enframe(degree(x, rescale = TRUE))      %>% 
	mutate(userid = userid$userid[name]) 	%>%
	select(userid, value)			        		%>%
	rename(degree = value)				      	%>%
	arrange(desc(degree))					        %>%
...
vertices <- enframe(cohort(Qall)) %>% rename(userid = value) %>% select(-name)
prominence <- bind_cols(vertices, prominence)
prominence <- prominence %>% arrange(desc(deg, ldc, evc))
...
All of the functions described in this section, `degree`, `betweenness` and `stresscent` have been run with the `rescale = TRUE` option to formalize them.
...
```{r, echo=FALSE}
kable(betw(Qall,10)) %>%   kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
ggnet2(Qall, label = betw(Qall,10)$userid, label.size = 4, label.color = "darkblue", node.size = 0.25, edge.size = 0.25, edge.color = "grey")
top_betw <- betw(Qall,10)$userid
```

### Stress centrality

This measures the extent to which vertices lie on high numbers of the shortest paths between other vertices.

```{r}
kable(str_cent(Qall,10)) %>%   kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
ggnet2(Qall, label = str_cent(Qall,10)$userid, label.size = 4, label.color = "darkblue", node.size = 0.25, edge.size = 0.25, edge.color = "grey")
top_stress <- str_cent(Qall,10)$userid
```
...
### Usefulness and combination measures

Which of these measures to privilege is not clear. They each present different perspectives of the relative importance of userd in the network, based on different criteria, but none presents an obvious candidate by itself. The union of users identified by these methods is

```{r, echo = FALSE}
central_users <- (union(union(top_deg,top_betw),top_stress))
kable(central_users) %>%  kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
ggnet2(Qall, label = central_users, label.size = 4, label.color = "darkblue", node.size = 0.25, edge.size = 0.25, edge.color = "grey")
```
which indicates some overlap, and user 1846, the star of the naive top-10 lists failed to make any of the formal top-10 lists.



### Latent network analysis

To explicate the results, a machine learning tool is indicated.  The [latentnet] package was selected for that task. An [accessible] description of that package, as well as a more [theoretical] paper are available for orientation. 
...
The first step is to fit^[This is a long-running function, which may take 45 minutes or more even with 32GB memory. The chunk that follows loads an Rda object of the model stored in an AWS S3 bucket.] a model to a network object. `Qall` will be used. 

    Qall.fit <- ergmm(Qall ~ euclidean(d=2, G=3), verbose = TRUE)
...
```{r, echo = FALSE}
# Qall.fit <- ergmm(Qall ~ euclidean(d=2, G=3), verbose = TRUE)
# save(Qall.fit, file = "Qall.fit.Rda")
#con("https://s3-us-west-2.amazon.aws/dslabs-nostromo/Qall.fit.Rda")
#load(con)
#close(con)
load("Qall.fit.Rda")
```
...
```{r}
# ![Trace diagnostic of .fit](Qall.fit_trace.png)
...
```{r}
load("Qall.fit.Rda")
mcmc.diagnostics(Qall.fit, which.diags = c("cor", "acf", "rafferty"))
```
...
```{r}
Qall.gof <- gof(Qall.fit)
Qall.gof
par(mfrow=c(1,3))
par(oma=c(0.5,2,1,0.5))
plot(Qall.gof, plotlogodds=TRUE)
```

#### Visualization of model clusters, position within sample space and density

```{r}
plot(Qall.fit, pie=TRUE)
plot(Qall.fit, what="cloud", rand.eff="receiver", Z.ref=Z.ref, Z.K.ref=Z.K.ref)
plot(Qall.fit, what="density", rand.eff="receiver", Z.ref=Z.ref, Z.K.ref=Z.K.ref)
```

#### Model Diagnostics

Graphs do not always converge under the `ergmm` function call. To test this, a diagnostic is available to detect failure of the Markov chain Monte Carlo simulation to converge and to detect autocorrelation effects. One section provides an initial and lag 10 covariance matrix of the sampling. 
