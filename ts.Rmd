...
```{r, echo=FALSE}
kable(loadcent(Qall,rescale = TRUE) %>% sort(decreasing = TRUE) %>% head(10)) %>%   kable_styling(bootstrap_options = "striped", full_width = F, position = "center")
...
```{r, echo=FALSE}
central_users <- (union(union(top_deg,top_ldc),top_evc)) %>% sort()
...
deg_s <- prominence %>% select(userid, deg) %>% arrange(desc(deg)) %>% head(160)
ldc_s <- prominence %>% select(userid, ldc) %>% arrange(desc(ldc)) %>% head(160)
evc_s <- prominence %>% select(userid, evc) %>% arrange(desc(evc)) %>% head(160)
