...
At the time, electronic record examination (*ediscovery*) in litigation was in a primativeMuch of the focus was directed to keyword searches, sometimes called the *smoking gun* approach. Brute force examination misses opportunities to understand the social networks that reflect how the organization operates, what their concerns are and which part of the corpus should receive priority. To do that the corpus must be distilled and analyzed.

# Analysis

## Data acquisition

I obtained a copy of the [2009 version](https://www.cs.cmu.edu/~./enron/) of the corpus. It contains copies of emails of a private nature that involve three users have since requested to be [redacted](https://www.cs.cmu.edu/~./enron/DELETIONS.txt). I have removed those 27 emails.^[Most of my work on data wrangling and preliminary took place in 2018 in Python, relying heavily on the NLTK and networkx packages.]
...
Analysis of emails on the principal that "something may be overlooked"  delays reducing its collective informational value in preliminary analysis. Prioritizing always leaves open the option of reviewing the set-asides later.
...
For purposes of this paper, the SQL database has been serialized to an `Rda` [file](https://s3-us-west–2.amazonaws.com/dslabs.nostromo/enron.Rda) of approximately 10MB size, which can be brought into an `R` session with the following 
...
From the `sender`, `tos` and `ccs` fields, emails were extracted. Emails in the
*enron.com* were assigned a unique user identification, `uid`, integer; all
external domain addresses were assigned the uid of 9999.


```{r de-bracket recipients}
...
```{r identities}
# prepare list of email names
from_names <- split(enron$sender, ",") %>% str_trim()
to_names <- split(enron$tos, ",") %>% str_trim()
cc_names <- split(enron$ccs, ",") %>% str_trim()


all_names <- cbind(from_names, to_names, cc_names)
all_names <- gsub('c(\"',"", all_names, fixed = TRUE)
all_names <- gsub('\"', "", all_names, fixed = TRUE)
all_names <- gsub(' , , ', "", all_names, fixed = TRUE)
all_names <- gsub('(',"", all_names, fixed = TRUE)  
all_names <- gsub(',,', ",", all_names, fixed = TRUE)
all_names <- gsub('\\"',"", all_names)  
all_names <- gsub(new_line, "", all_names)
all_names <- gsub(",,,","", all_names)
all_names <- gsub(",,","", all_names)
...
all_names_c <- Corpus(VectorSource(all_names))
names <- as_tibble(scan_tokenizer(all_names_c))
unis <- names %>% unique()
census <- unis %>% group_by(value) %>% count() %>% arrange(-desc(n))
census <- census %>% filter(value != ',')
census <- census %>% filter(value != ',,')
census <- census %>% filter(value != 'list()')
census <- census %>% filter(value != 'list(language ')

 census <- census %>% ungroup() %>% mutate(value = str_replace(value,",$",""))
census <- census %>% mutate(value = str_replace(value,"^,",""))
colnames(census) <- c("address", "uid")
uids <- enframe(seq(10000,99999,1))
uids <- sample(uids$value, 10987, replace = FALSE)
uids <- enframe(uids)
colnames(uids) <- c("name", "uid")
census <- census %>% mutate(uid = uids$uid)
